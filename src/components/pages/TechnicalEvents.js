...
function TechnicalEvents() {
    ...
    const [modalEvent, setModalEvent] = useState(null); // Event to show in modal
    const navigate = useNavigate();

    useEffect(() => {
        const fetchEvents = async () => {
            try {
                const snapshot = await getDocs(collection(db, "technical_events"));
    ...
    return (
        <>
            <div className="row">
                {events.length === 0 ? (
                    <p className="text-center">Loading Technical Events...</p>
                ) : (
                    events.map((event) => (
                        <div key={event.id} className="col-lg-4 col-md-6 col-sm-12 mb-4">
                            <div className="card event-card h-100">
                                <div className="card-body d-flex flex-column">
                                    ...
                                    <p className="card-subtitle mb-2 text-muted">📅 {event.date}</p>
                                    ...
                                    <div className="mt-auto d-flex justify-content-between">
                                        <button
                                            ...
                                            onClick={() => setModalEvent(event)} // open modal
                                        >
                                            Show More
                                        </button>
                                        <button
                                            className="btn btn-primary"
                                            onClick={() => navigate("/register", { state: { eventName: event.title } })}
                                        >
                                            Register
                                        </button>
                                    </div>
